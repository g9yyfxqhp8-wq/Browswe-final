export default async function handler(req, res) {
  ...
  try {
    // Use global fetch (Node 18+ runtime)
    const response = await fetch(url, { headers: { "User-Agent": "Mozilla/5.0" } });
    const contentType = response.headers.get("content-type") || "";

    // Serve images, CSS, JS directly
    ...
    // Inject script to rewrite fetch/XHR in iframe
    const injectScript = `
      <script>
    ...
    res.status(500).send(`<h2>Failed to load site</h2><p>${err.message}</p>`);
  }
}
